...
namespace Obvs.Monitoring.ElasticSearch
{
    public class ElasticSearchMonitor<TMessage> : IMonitor<TMessage>
    {
        ...
        private readonly ElasticsearchClient/* IElasticClient*/ _client;
        ...
        public ElasticSearchMonitor(string instanceName, string counterName, string indexPrefix, IList<Type> types, TimeSpan samplePeriod, IScheduler scheduler, ElasticsearchClient /* IElasticClient*/ client)
        {
        ...
        private void Save(IList<ObvsCounter> items)
        {
            try
            {
                if (items.Any())
                {
                    ...
                    if (!response.IsSuccess() /*IsValid*/ && response.Errors /*ServerError != null*/)
                    {
                        if (response.TryGetElasticsearchServerError(out ElasticsearchServerError serverError))
                            Console.WriteLine(serverError /*response.ServerError.Error*/);
                        else
                            Console.WriteLine("Undefined Elasticsearch Server Error");
                    }
                }
            }

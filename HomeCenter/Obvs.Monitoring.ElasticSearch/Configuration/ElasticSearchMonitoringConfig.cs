...
namespace Obvs.Monitoring.ElasticSearch.Configuration
{
        ...
        where TResponse : class, TMessage
    {
        ...
        ICanAddEndpointOrLoggingOrCorrelationOrCreate<TMessage, TCommand, TEvent, TRequest, TResponse> ConnectToServer(NodePool /*IConnectionPool*/ elasticConnectionPool);
    }
        ...
        where TResponse : class, TMessage
    {
        ...
        public ICanAddEndpointOrLoggingOrCorrelationOrCreate<TMessage, TCommand, TEvent, TRequest, TResponse> ConnectToServer(string uri)
        {
            var monitorFactory = new ElasticSearchMonitorFactory<TMessage>(
                ...
                new ElasticsearchClient /* ElasticClient*/(new ElasticsearchClientSettings /*ConnectionSettings*/(new Uri(uri))));

            _config.UsingMonitor(monitorFactory);
            return _config;
        }

        public ICanAddEndpointOrLoggingOrCorrelationOrCreate<TMessage, TCommand, TEvent, TRequest, TResponse> ConnectToServer(NodePool /*IConnectionPool*/ elasticConnectionPool)
        {
            var monitorFactory = new ElasticSearchMonitorFactory<TMessage>(
                _indexPrefix, _types, _instanceName,
                _samplePeriod, Scheduler.Default,
                new ElasticsearchClient/* ElasticClient*/(new ElasticsearchClientSettings /* ConnectionSettings*/(elasticConnectionPool)));
